...
class Field(object):
    ...
    def __init__(self, validate=None, default=None, prepare=None, clean=None,
            hide_value=False, locale=None, tz=None, **kwargs):
    ...
    def load_data(self, str_value=None, obj_value=None, file_data=None,
            locale=None, tz=None):
    ...
    def to_string(self, *args):
        if self.hide_value:
            return u''
        str_value = self.get_str_value(*args)
        return self.prepare_value(str_value, *args)

    def get_str_value(self, *args):
        if self.str_value is None:
            return self.py_to_str(*args)
        return self.str_value

    def py_to_str(self, *args):
        return to_unicode(self.obj_value or u'')

    def prepare_value(self, str_value, *args):
        if not self.prepare:
            return str_value
        return self.prepare(str_value, *args)
    

    def to_python(self, *args):
        try:
            return self.str_to_py(*args)
    ...
    def validate(self, form=None, cleaned_data=None, *args):
        if cleaned_data is None:
            return self.validate_field(form, *args)
        self.validate_form(form, cleaned_data)

    def validate_field(self, form, *args):
        self.error = None
        py_value = self.to_python(*args)
        ...
        py_value = self.clean_value(py_value, *args)
        self.has_changed = (py_value != self.obj_value)
        return py_value

    def clean_value(self, py_value, *args):
        if not self.clean:
            return py_value
        try:
            return self.clean(py_value, *args)
        except ValidationError, error:
            self.error = error
            return None
